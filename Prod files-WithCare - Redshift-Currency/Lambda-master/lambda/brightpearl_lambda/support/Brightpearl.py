...
class BrightpearlModule(Enum):
    ...
    CUSTOMFIELD = {"Id":"contactId"
               ,"databaseId":"segmentaid"
               ,"search_reference":"/contact-service/contact-search?tagIds=9"
               ,"single_reference":"/contact-service/contact/{id-set}/custom-field"
               ,"set_retrieval":"/contact-service/contact-search?tagIds=9"
               ,"table_name": "customfield"
               ,"is_simple" : True
               ,"is_bridge_table" : False
               ,"post_filter": {}
               ,"has_hard_deletes": False
    ...
    # CUSTOMFIELD = {"Id":"Id"
               # ,"databaseId":"segmentaid"
               # ,"search_reference":"/contact-service/contact/"
               # ,"single_reference":"/contact-service/contact/{id-set}/custom-field"
               # ,"set_retrieval":"/contact-service/contact/"
               # ,"table_name": "customfield"
               # ,"is_simple" : True
               # ,"is_bridge_table" : True
               # ,"post_filter": {"table_reference": "contacts", "id_reference" : "contactid"}
               # ,"has_hard_deletes": False
                # }
    ...
    CURRENCY = {"Id":"id"
               ,"databaseId":"Id"
               ,"search_reference":"/accounting-service/currency-search"
               ,"single_reference":"/accounting-service/currency-search"
               ,"set_retrieval":"/accounting-service/currency-search"
               ,"table_name": "currency"
               ,"is_simple" : True
               ,"is_bridge_table" : False
               ,"post_filter": {}
...
class BrightpearlAPIInteraction:
            ...
            while more_results == True:
                ...
                if tablename=="customerpayment":     
                                                parameters = {"columns":module_info["Id"]
                                                                            ,"createdOn": timestamp + "/" +now#>yeti max inventory date
                    ...
                    parameters = {"columns":module_info["Id"]
                                                ,"createdOn": '1970-01-01T00:00:01' + "/" +now#>yeti max inventory date
                                                ,"firstResult": current_result}
    ...
    def get_details_from_entity (self, module_segment, entity_list, database_service=None):
        ...
        if module_info["table_name"]!="customfield":
            ...
            for i in range(0, len(entity_list)):
                entity_list[i] = str(entity_list[i])
            #res = [''.join(ele) for ele in entity_list]
            res = list(map(''.join, entity_list))
            for i in range(0, len(res)):
                res[i] = res[i][1:-1]
            work_list_for_ids = res
        print(work_list_for_ids)
        ...
        try:
                ...
                if is_simple == False:
                    ...
                    if len(string_of_ids) > 0:
                        ...
                        if "errors" in result:
                            ...
                            if module_info["table_name"]=="customfield":
                                result["response"]["contactid"] = string_of_ids
            ...
            # for item, id_ in zip(list_of_entities_to_return, work_list_for_ids):
                # item['contactid'] = id_
                # item.append({"orderid":id_})
...
class BrightpearlIntegrationInteraction:
    ...
    def __mapping_dbparams_to_response (self, list_with_details):
            ...
            if self.entity is BrightpearlModule.PRODUCTAVAILABILITY:
            ...
            elif self.entity is BrightpearlModule.CUSTOMFIELD:

                print(list_with_details)
                for customfield in list_with_details:

                    entities_to_insert.append({
                                "contactid" : customfield["contactid"] if "contactid" in customfield else 0
                                ,"segmentaid" : customfield["PCF_SEGMENTA"]["id"] if "id" in customfield["PCF_SEGMENTA"] else 0
                                ,"segmentavalue": customfield["PCF_SEGMENTA"]["value"] if "value" in customfield["PCF_SEGMENTA"] else "NA"
                                ,"createdon": datetime.fromtimestamp(time.time()).strftime("%Y-%m-%d %H:%M:%S")
                                ,"updatedon": datetime.fromtimestamp(time.time()).strftime("%Y-%m-%d %H:%M:%S")
                        })
                ...
                for contact in list_with_details:
                    ...
                    entities_to_insert.append({
                                ...
                                ,"countryiso" : contact["postalAddresses"][0]["countryIsoCode"] if "postalAddressed" in contact else "NA"
            ...
            elif self.entity is BrightpearlModule.CURRENCY:

                for cur in list_with_details:
                    for values in cur['results']:
                            entities_to_insert.append({
                                "currencyid" :values[0],
                                "currencyname" :values[1],
                                "currencyisocode" :values[2],
                                "currencysymbol" :values[3],
                                "exchangerate" :values[4]
                                ,"createdon": datetime.fromtimestamp(time.time()).strftime("%Y-%m-%d %H:%M:%S")
                                ,"updatedon": datetime.fromtimestamp(time.time()).strftime("%Y-%m-%d %H:%M:%S")
                                })
    ...
    def entity_soft_delete (self):
        ...
        end_time = datetime.time(datetime(1970,1,1,00,40,00)) 
        ...
        if time_now >= start_time and time_now < end_time:
            ...
            list_of_ids_brightpearl = self.brightpearl_api_interface.get_ids_from_entity(self.entity, "1970-01-01T00:00:00")

            # List of ids in the database
            list_of_ids_database = db1.get_ids_on_timestamp("1970-01-01 00:00:00") # This way we'll get all the ids that exist in the db that weren't deleted yet
