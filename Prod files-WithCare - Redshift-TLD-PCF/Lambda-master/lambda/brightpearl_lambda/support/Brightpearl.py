...
class BrightpearlModule(Enum):
    ...
    PRODUCTCUSTOMFIELD = {"Id":"productId"
               ,"databaseId":"Id"
               ,"search_reference":"/product-service/product-search"
               ,"single_reference":"/product-service/product-custom-field/{id-set}"
               ,"set_retrieval":"/product-service/product-search"
               ,"table_name": "productcustomfield"
               ,"is_simple" : True
               ,"is_bridge_table" : False
               ,"post_filter": {}
               ,"has_hard_deletes": False
...
class BrightpearlAPIInteraction:
    ...
    def __init__ (self, logger, app_reference, account_name, account_token, schema_to_handle):
        ...
        # self.schema_to_handle = "bp_etl_development"
    ...
    def get_ids_from_entity(self, module_segment, timestamp, database_service=None):
        ...
        if module_info["is_bridge_table"] == False:                                
            ...
            while more_results == True:
                ...
                elif tablename=="channel" or tablename=="warehouse" or tablename=="productcustomfield":
                    print("Not in CP")
                    ...
                    if len(string_of_ids) > 0:
                        ...
                        if "errors" in result:
                            ...
                            if len(result["response"]) > 0:
                                ...
                                # if type(result["response"]) is dict:
                                    # print("In dict")
                                    # #print(result["response"])
                                    # list_of_entities_to_return.append(result["response"])
                                for entities in result["response"]:
...
class BrightpearlIntegrationInteraction:
    ...
    def __mapping_dbparams_to_response (self, list_with_details):
            ...
            if self.entity is BrightpearlModule.PRODUCTAVAILABILITY:
            ...
            elif self.entity is BrightpearlModule.PRODUCTCUSTOMFIELD:
                ...
                for product in list_with_details:
                    ...
                    for values in product:
                        ...
                        entities_to_insert.append({
                                    "seasonid" : product[values]["PCF_SEASON"]["id"] if "PCF_SEASON" in product[values] else 0
                                    ,"seasonname" : product[values]["PCF_SEASON"]["value"] if "PCF_SEASON" in product[values] else "NA"
                                    ,"genderid" : product[values]["PCF_GENDER"]["id"] if "PCF_GENDER" in product[values] else 0
                                    ,"gendername" : product[values]["PCF_GENDER"]["value"] if "PCF_GENDER" in product[values] else "NA"
                                    ,"createdon": datetime.fromtimestamp(time.time()).strftime("%Y-%m-%d %H:%M:%S")
                                    ,"updatedon": datetime.fromtimestamp(time.time()).strftime("%Y-%m-%d %H:%M:%S")
                                    ,"productid" : values 
                            })
